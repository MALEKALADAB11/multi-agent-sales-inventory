"""
Unified API Server - Inventory + Sales + Agents IA + Monitoring
================================================================
Run with:
...
app = FastAPI(
    ...
    description="Inventory + Sales + Agents IA + Monitoring",
    version="2.0.0",
)
...
# NOTE: coach_chat_rag router intentionally NOT included here.
# The /api/v1/coach/chat endpoint below is richer (real weather,
# STORE_MAP resolution, full product catalogue) and supersedes it.
...
async def health():
    ...
    return {
        "status":  "ok",
        "version": "2.0.0",
        "modules": ["inventory", "sales", "agents-ia", "rag", "monitoring"],
        "last_cycle": {
...
async def get_forecast_hourly_endpoint(store_id: str):
    """Retourne le CA réel par heure (0-23) depuis l'historique de transactions."""
    mapped_id   = STORE_MAP.get(store_id, "OOR_LAC_01")
    provider    = get_data_provider()
    pos_history = await provider.fetch_pos_history(mapped_id)
    ...
    for tx in pos_history:
        ...
        if tx_time and hasattr(tx_time, "hour"):
            h = tx_time.hour
            hourly[h] = hourly.get(h, 0.0) + tx.get("revenue", 0.0)

    data   = [round(hourly.get(h, 0)) for h in range(24)]
...
async def simulate_pos_endpoint(store_id: str):
    """Déclenche une vente simulée via le RealtimeSimulator existant."""
    simulator = getattr(app.state, "simulator", None)
    if simulator:
        return JSONResponse({
            "status":  "success",
    ...
    return JSONResponse(
        {"status": "error", "message": "Simulator not running"},
        status_code=503,
    )

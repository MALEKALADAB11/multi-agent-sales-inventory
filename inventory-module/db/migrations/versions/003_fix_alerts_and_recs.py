...
def upgrade() -> None:
    ...
    # ── inv.alerts: add decided_by column ────────────────────────────────────
    op.execute("""
        ALTER TABLE inv.alerts
        ADD COLUMN IF NOT EXISTS decided_by VARCHAR(100)
    """)

    # ── inv.alerts: add 'rejected' and 'acknowledged' to status constraint ───
    ...
    # ── inv.alerts: add 'below_minimum' to alert_type constraint ─────────────
